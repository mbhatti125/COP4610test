...
static void bitmap_init(int start, int num, int nbits){
	...
	// loop through each sector in bitmap & initialize it to 0
...
static int bitmap_first_unused(int start, int num, int nbits){
	int sec, byte, bit;
	char buf[SECTOR_SIZE], res;
	int index = 0;

	// loop through all sectors in bitmap
	for (sec = 0; sec < num; sec++){
		Disk_Read(start + sec, buf);
		for (byte = 0; byte < SECTOR_SIZE; byte++){
			for (bit = 7; bit >= 0; bit--){	// loop through every bit
				res = buf[byte] >> bit;
				res &= 1;
				if (res == 0){  // if used bit found, set to 0 and write changes
					buf[byte] |= 1 << bit;
					Disk_Write(start + sec, buf);
					return index;
				}
...
static int bitmap_reset(int start, int num, int ibit) {
	...
	for (sec = 0; sec < num; sec++){
		Disk_Read(start + sec, buf);
		for (byte = 0; byte < SECTOR_SIZE; byte++){
			for(bit = 7; bit >= 0; bit--){  // loop through each bit in bitmap
...
static int illegal_filename(char* name) {
	...
	if(length >= MAX_NAME-1){
		printf("Failing Length\n");
		return 1;
	}
	
	//check to make sure each character is not illegal
	for(i = 0; i < length; i++){
		if(!isalpha(name[i]) && !isdigit(name[i]) && !strchr(good_chars, name[i])){
			printf("Failing %c\n", name[i]);
			return 1;
		}
	}

	//return success if no error as found 
	return 0;
}
...
static int find_child_inode(int parent_inode, char* fname, int *cached_inode_sector, char* cached_inode_buffer) {
	...
	int nentries = parent->used; // remaining number of directory entries 
	int idx = 0;
	while(nentries > 0) {
...
int add_inode(int type, int parent_inode, char* file) {
	...
	int group = parent->used/DIRENTS_PER_SECTOR;
	char dirent_buffer[SECTOR_SIZE];
	if(group*DIRENTS_PER_SECTOR == parent->used) {
	...
	offset = parent->used-start_entry;
...
int File_Read(int fd, void* buffer, int size) {
	...
	assert(0 <= offset && offset < INODES_PER_SECTOR);
	inode_t* file = (inode_t*)(inode_buffer+offset*sizeof(inode_t));

	// variables used to read
	char buf[SECTOR_SIZE];
	char* data = (char*) buffer;
	int i, j;
	int spos = open_files[fd].pos;
	int count = 0;

	// define start variables to read starting from file pointer
	int start = open_files[fd].pos / SECTOR_SIZE;
	int startb = open_files[fd].pos % SECTOR_SIZE;

	// loop through sectors in file's data array
	for(i = start; i < MAX_SECTORS_PER_FILE; i++){
		if (file->data[i]){
			Disk_Read(file->data[i], buf);
			dprintf("Buf contents: %s", buf);
			for(j = startb; j < SECTOR_SIZE; j++){	// loop through bytes
				if (count+spos < file->size && count < size){	// while still inside of the file & reading into size of buffer
					data[count++] = buf[j];
				}
			}
			//reset starting byte after first sector
			startb = 0;
		}
	}
	// move forward the file counter
...
int File_Write(int fd, void* buffer, int size) {
	...
	for(i = start; i < MAX_SECTORS_PER_FILE && count < size; i++){
		//if we're writing in already allocated sectors
		if (file->data[i]){
			Disk_Read(file->data[i], buf);
			for(j = startb; j < SECTOR_SIZE; j++){
				// write data to buffer
				if(tempSize-- > 0) buf[j] = data[count++];
			}
			startb = 0;
			dprintf("SECTOR %d: %s\n", file->data[i], buf);
			Disk_Write(file->data[i], buf);
		}
		else {
			//allocate new sector 
			int newsec = bitmap_first_unused(SECTOR_BITMAP_START_SECTOR, SECTOR_BITMAP_SECTORS, SECTOR_BITMAP_SIZE);
			...
			file->data[i] = newsec;
			char newbuf[SECTOR_BITMAP_SIZE];
			memset(newbuf, 0, SECTOR_SIZE);
			for(j = 0; j < SECTOR_SIZE; j++){
				// write to buffer
				if(tempSize-- > 0) newbuf[j] = data[count++];
			}
			//save changes
	...
	//save changes to open file table + write changes to disk
	open_files[fd].pos += size;
	file->size += size;
	open_files[fd].size += size;
	Disk_Write(inode_sector, inode_buffer);
...
int File_Seek(int fd, int offset) {
	...
	//check if file pointer is within the file
	if (open_files[fd].size < offset || offset < 0){
...
int Dir_Size(char* path) {
	...
	if(!dir->type){
		osErrno = E_GENERAL;
		dprintf("Dir inode info: type-%d inode-%d size-%d\n", dir->type, d_inode, dir->size);
		return -1;
	}

	//return the size of all dirents
